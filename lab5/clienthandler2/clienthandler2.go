...
func NewClientHandler(id int, proposer *multipaxos.Proposer, ld detector.LeaderDetector, clConnChan chan *net.TCPConn) *ClientHandler {
	return &ClientHandler{
		id:                  id,
		proposer:            proposer,
		ClientValueChanIn:   make(chan multipaxos.Value, 429496),
		ClientTimeoutChanIn: make(chan multipaxos.Value, 429496),
		LearnValueChan:      make(chan multipaxos.Value, 429496),
		responseChanIn:      make(chan multipaxos.Response, 429496),
		ld:                  ld,
		leader:              ld.Leader(),
