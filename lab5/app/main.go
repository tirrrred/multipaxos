...
func main() {
	...
	hbSend := make(chan detector.Heartbeat, 429496)
	...
	if iAMa(netconf.Proposers, appnet.Myself.ID) { //If I'am a proposer
		...
		prepareOutChan = make(chan multipaxos.Prepare, 429496)
		acceptOutChan = make(chan multipaxos.Accept, 429496)
	...
	if iAMa(netconf.Acceptors, appnet.Myself.ID) { //If I'am a acceptor
		...
		promiseOutChan = make(chan multipaxos.Promise, 429496)
		learnOutChan = make(chan multipaxos.Learn, 429496)
	...
	if iAMa(netconf.Learners, appnet.Myself.ID) { //If I'am a learner
		...
		decidedOutChan = make(chan multipaxos.DecidedValue, 429496)
	...
	responseOutChan := make(chan multipaxos.Response, 429496)
