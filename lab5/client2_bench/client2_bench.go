...
var (
	...
	numReq         = flag.Int(
		"requests",
		500,
		"Specify number of random request for benchmarking",
	)
)
...
func randomReq() string {
	...
	account := rand.Intn(1000)
	if operation == 0 { //balance
		return "balance " + strconv.Itoa(account)
	}
	if operation == 1 {
		return "deposit " + strconv.FormatInt(amount, 10) + " " + strconv.Itoa(account)
	}
	return "withdraw " + strconv.FormatInt(amount, 10) + " " + strconv.Itoa(account)
}
